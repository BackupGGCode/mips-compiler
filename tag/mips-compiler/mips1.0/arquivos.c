...
{
    /* Variaveis locais */
    int len;                    //len é o tamanho da string
    int k;                      //k o indice para percorre-la
    int n;                      //n é o bit atual
    unsigned int sum = 0;       //sum é a soma dos bits
    unsigned int des;           //des é o valor do numero deslocado
...
{
    //Verifica se o arquivo não é NULL
    if (arq == NULL)
    {
    ...
    unsigned int numAtual;  // Armazena o valor decimal da linha
    int erro = 0;           // Variavel de controle para o erro
